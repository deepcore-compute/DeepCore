...
inline std::string generate_progpowz_cuda_round_source(const std::vector<TraceOp>& trace)
{
    ...
    for (const auto& op : trace)
    {
        ...
        {
            // Scoped in its own block: each round emits up to
            // num_math_operations (20) of these, and without a nested
            // scope every `uint32_t _math = ...;` would redeclare the
            // same name in the enclosing round block - a real bug caught
            // by manually reading generated output (see this project's
            // engineering-discipline notes on why review of generated
            // code, not just its structural shape, matters even when a
            // full compile isn't possible in this environment).
            out << "        {\n            uint32_t _math = ";
            switch (op.math_case)
            {
...
// Emits a COMPLETE, standalone .cu source string for one period,
// implementing the same lane-cooperative warp-shuffle algorithm as
// progpowz_warp_kernel/progpowz_hash_warp_full_dag in progpowz_kernel.cu,
// but with the per-round mix program fully resolved (via
// generate_progpowz_cuda_round_source() above) instead of interpreted at
// runtime - so, unlike progpowz_warp_kernel, this kernel takes NO
// block_number parameter at all: the period is baked into which compiled
// kernel gets launched (see progpowz_nvrtc_kernel.hpp), not passed as
// data.
//
// Meant to be compiled via NVRTC with `progpowz_portable.hpp`'s actual
// file content supplied as a named header (see
// progpowz_nvrtc_kernel.cpp) - this reuses the exact same, already
// CPU-and-GPU-validated primitives (load_le32, fnv1a, keccak_progpow_64/
// 256, kiss99_next, the hash256/hash2048/GpuHashResult/progpowz_result
// types) rather than re-transcribing them into a string, which would be
// a second, divergence-prone copy of already-proven code.
//
// The kernel entry point is declared `extern "C"` specifically so its
// exported symbol name is the literal `progpowz_nvrtc_warp_kernel`, not a
// C++-mangled name - the host-side loader looks it up by that exact
// string (see cuModuleGetFunction in progpowz_nvrtc_kernel.cpp).
inline std::string generate_progpowz_nvrtc_kernel_source(int block_number)
{
    const auto trace = generate_progpowz_trace(block_number);
    const std::string round_body = generate_progpowz_cuda_round_source(trace);

    std::ostringstream src;
    src << "// Generated by progpowz_codegen.hpp for block_number=" << block_number
        << " (period " << (block_number / period_length) << "). DO NOT EDIT BY HAND.\n"
        << "#include \"progpowz_portable.hpp\"\n"
        << "using namespace deepcore::progpowz;\n\n"
        << "__device__ inline progpowz_result progpowz_hash_warp_full_dag_generated(\n"
        << "    const uint32_t* __restrict__ l1_cache_words, uint32_t full_dataset_num_items,\n"
        << "    const hash2048* __restrict__ full_dataset, const hash256& header_hash, uint64_t nonce,\n"
        << "    unsigned mask, int warp_lane, int lane)\n"
        << "{\n"
        << "    const int group_base_lane = warp_lane - lane;\n"
        << "    const uint64_t seed = keccak_progpow_64(header_hash, nonce);\n"
        << "    uint32_t r[num_regs];\n"
        << "    {\n"
        << "        const uint32_t z = fnv1a(fnv_offset_basis, (uint32_t)seed);\n"
        ...
        << "        kiss99_state rng{z, w, jsr, jcong};\n"
        << "        for (uint32_t i = 0; i < num_regs; ++i) r[i] = kiss99_next(rng);\n"
        << "    }\n"
        << round_body
        << "    uint32_t lane_hash = fnv_offset_basis;\n"
        << "    for (uint32_t i = 0; i < num_regs; ++i) lane_hash = fnv1a(lane_hash, r[i]);\n"
        << "    uint32_t mh[8];\n"
        << "    for (int i = 0; i < 8; ++i) mh[i] = fnv_offset_basis;\n"
        << "    for (int l = 0; l < (int)num_lanes; ++l)\n"
        << "    {\n"
        ...
        << "    return out;\n"
        << "}\n\n"
        << "extern \"C\" __global__ void progpowz_nvrtc_warp_kernel(\n"
        << "    const uint32_t* __restrict__ l1_cache_words, uint32_t full_dataset_num_items,\n"
        << "    const hash2048* __restrict__ full_dataset, hash256 header_hash, uint64_t start_nonce,\n"
        ...
        << "    const int lane = (int)(global_thread_idx % num_lanes);\n"
        << "    const int warp_lane = (int)(threadIdx.x % 32);\n"
        << "    const uint64_t nonce = start_nonce + nonce_group;\n"
        << "    constexpr unsigned kFullWarpMask = 0xFFFFFFFFu;\n\n"
        << "    progpowz_result r = progpowz_hash_warp_full_dag_generated(\n"
        << "        s_l1_cache, full_dataset_num_items, full_dataset, header_hash, nonce, kFullWarpMask, "
           "warp_lane, lane);\n\n"
